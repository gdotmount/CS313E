...
def array220(nums, index):
    if len(nums) < 2 or index >= len(nums):
        ...
        return nums[index] * 10 in nums[index:] or array220(nums, index + 1)
...
def strCopies(str, sub, n):
    if n != -1:
        return n == strCopies(str, sub, -1)
    elif len(sub) > len(str):
        return 0
    elif str[:len(sub)] == sub:
        return 1 + strCopies(str[1:], sub, -1)
    else:
        return strCopies(str[1:], sub, -1)
...
def strDist(str, sub):
    if len(str) < len(sub):
        return 0
    elif str[:len(sub)] == sub and str[-1 * len(sub):] == sub:
        return len(str)
    elif str[:len(sub)] == sub and str[-1 * len(sub):] != sub:
